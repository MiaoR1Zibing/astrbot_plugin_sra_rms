...
title: "03 - 监听消息事件"
date: 2026-06-17
description: AstrBot 消息与事件处理详解，包括指令、过滤器、事件钩子等
---
...
- [事件钩子](#事件钩子)
  ...
  - [等待 LLM 请求时](#等待-llm-请求时)
  - [LLM 请求时](#llm-请求时)
  - [LLM 请求完成时](#llm-请求完成时)
  - [Agent 开始运行时](#agent-开始运行时)
  - [LLM 工具调用前](#llm-工具调用前)
  - [LLM 工具调用后](#llm-工具调用后)
  - [Agent 运行完成时](#agent-运行完成时)
  - [发送消息前](#发送消息前)
  - [发送消息后](#发送消息后)
- [优先级](#优先级)
- [控制事件传播](#控制事件传播)
...
> **提示**: 指令不能带空格，否则 AstrBot 会将其解析到第二个参数。可以使用下面的指令组功能，或者也使用监听器自己解析消息内容。
...
> **提示**: 事件钩子不支持与上面的 @filter.command, @filter.command_group, @filter.event_message_type, @filter.platform_adapter_type, @filter.permission_type 一起使用。
...
- 原始文档: https://docs.astrbot.app/dev/star/guides/listen-message-event.html
- 整理时间: 2026-06-17
