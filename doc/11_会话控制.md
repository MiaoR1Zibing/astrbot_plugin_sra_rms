...
title: "11 - 会话控制"
date: 2026-06-17
description: AstrBot 会话控制器详解，实现多轮对话交互
---
...
考虑一个成语接龙插件，某个/群用户需要和机器人进行多次对话，而不是一次性的指令。这时候就需要会话控制。
...
| 方法 | 说明 |
|------|------|
| `keep(timeout, reset_timeout)` | 保持这个会话。reset_timeout=True 时重置超时时间 |
| `stop()` | 结束这个会话 |
...
- 原始文档: https://docs.astrbot.app/dev/star/guides/session-control.html
- 整理时间: 2026-06-17
