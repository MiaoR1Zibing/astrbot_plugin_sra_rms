...
| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| SRA Server 地址 | SRAFrontend.Server 的地址 | localhost |
| SRA Server 端口 | SRAFrontend.Server 的端口 | 5073 |
| 启用指令白名单 | 开启后仅白名单用户可用 | false |
| 指令白名单 | 平台 / 消息类型 / 账号 三维匹配 | 空 |
...
| 指令 | 说明 | 示例 |
|------|------|------|
| `/sra run [配置]` | 运行任务。不带参数加载默认配置 "default"；传序号需先查看 configs；传空串运行全部 | `/sra run` `/sra run 2` `/sra run farm` |
